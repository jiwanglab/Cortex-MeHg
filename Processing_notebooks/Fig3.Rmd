...
#Fig3d
```{r}
top5_merge <- cluster_markers_mixed %>% group_by(cluster) %>% top_n(n = 5, wt = avg_logFC)    
avg <- AverageExpression(Mixed, features = NULL, add.ident = NULL, return.seurat = TRUE, verbose = TRUE)
DoHeatmap(avg, features = top5_merge$gene, draw.lines = 0, label = F)+ theme(legend.text=element_text(size=20))
```
